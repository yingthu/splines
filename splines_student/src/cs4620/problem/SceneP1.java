...
public class SceneP1 extends JFrame implements GLSceneDrawer, ChangeListener {
	...
	public SceneP1() {
		super("CS 4620 Scene Assignment / Problem 1");
	...
	private void initMeshes(GL2 gl)
	{
		...
		meshes.add(new Cube(gl));
		try
		{
			meshes.add(new CustomTriangleMesh(gl, new File("data/meshes/bunny500.msh")));
			meshes.add(new CustomTriangleMesh(gl, new File("data/meshes/ship.msh")));
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		// WARNING: this method *must* be called by a thread that will not
		// run concurrently with other GL commands! For example, directly
		// calling this method from stateChanged() when the tolerance slider
		// is updated can cause bizarre bugs, because callbacks for
		// interface-related events are not guaranteed to be mutually exclusive
		// with the thread(s) that manipulate GL state.
