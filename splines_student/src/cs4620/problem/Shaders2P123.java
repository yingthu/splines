 ...
 */
public class Shaders2P123 extends JFrame implements GLPickingDrawer,
...

{
	...
	public Shaders2P123() {
		super("CS 4620 Shaders Assignment 2 / Problem 1, 2, 3");
...
//		shaderComboBox.setVisible(false);
	...
	public void init(GLAutoDrawable drawable, CameraController cameraController) {
		...
		mat.setDiffuse(1f,  1f,  1f);
		scene = new Scene(gl, mat);
	...
	protected void addNewShape(Mesh mesh, String name)
	{
		...
		try
		{
			...
			scene.addNewShape(path, mesh, name, mat);
			sliderChanged = true;
			refresh();
		}
		catch (Exception e) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		ActionPerformedShaders2P123Runnable runnable = new ActionPerformedShaders2P123Runnable(this, e, filename);
	...
	public void processAction(GL2 gl, ActionEvent e, String filename)
	{
		...
		else if (cmd.equals(SHADER_COMBO_BOX_COMMAND))
		{
			...
			if(nodes[0] instanceof MeshNode)
			{
				...
				else if(selectedShader.equals(TEXTURE_SHADER_TEXT) && !(meshMaterial instanceof TextureMaterial))
				{
					newLightableMaterial = new TextureMaterial();
					newLightableMaterial.setDiffuse(1f, 1f, 1f);
					meshNode.setMaterial(newLightableMaterial);
					changedMaterial = true;
				}
	...
	private void showHideSettingPanels(SceneNode[] selection)
	{
		...
		if (selection.length == 1)
		{
			...
			if (node instanceof MeshNode)
			{
				...
				else if(mat instanceof TextureMaterial)
				{
...
class ActionPerformedShaders2P123Runnable implements GLRunnable {
	
	Shaders2P123 problem;
	ActionEvent e;
	String filename;
	
	public ActionPerformedShaders2P123Runnable(Shaders2P123 problem, ActionEvent e)
	{
	...
	public ActionPerformedShaders2P123Runnable(Shaders2P123 problem, ActionEvent e, String filename)
	{
		this.problem = problem;
		this.e = e;
